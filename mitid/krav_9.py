    ...
    try:
        ...
        for i in range(1, 11):
            with tempfile.TemporaryDirectory(delete=0) as tmpdir: # Will produce temprary folders with naming convention of starting with "tmp" along with randomized suffixes
                ...
                backend.extract_known_databases("databaser/AFU.zip", r".*mitid.*.\.db.*", f"{tmpdir}/AFU")
                print("\nExtracting BFU databases")
                backend.extract_known_databases("databaser/BFU.zip", r".*mitid.*.\.db.*", f"{tmpdir}/BFU")
                print("\nExtracting adb databases")
                backend.extract_known_databases("databaser/adb.tar", r".*mitid.*.\.db.*", f"{tmpdir}/adb")
