...
ALL_REMA1000_DB = (
    r".*rema1000.*.\.db.*"  # Find all (.db, -wal, -shm, -journal) related to Rema1000.
)
...
if __name__ == "__main__":
    try:
        with tempfile.TemporaryDirectory(
            delete=0
        ) as tmpdir:  # pass delete=False in TemporaryDirectory() to keep the files to inspect, in case output is unexpected
            backend.extract_known_databases(AFU, REMA1000_RECEIPT_DB, tmpdir)
            databases = backend.list_dir_recursively(
                tmpdir
            )  # returns a list, but i just want the content, ergo indexing
            matches = []
            for database in databases:
                ...
                if match:
                    matches.append(
                        match.group()
                    )  # Creates a list with rel. databases in it
            receipts_db = matches[0]  # Only 1 item in the list.
            receipt_table_name = backend.get_db_tables(
                receipts_db
            )[0] # I want to access the first table in the db
